...
import { StyleSheet, FlatList, Pressable, Platform, TouchableOpacity, View, Dimensions, ImageBackground, Text } from 'react-native';
...
const VideoNavigator = ({album, SheetRef, navigation, check, setCheck, selectedVideos, setSelectedVideos, videosComponent, setVideosComponent, videoLoading, setVideoLoading}) => {
  ...
  useEffect(() => {
    ...
    getData()
    console.log("Video", videosComponent)
  }, []);
  ...
  //   //get sounds of album
  //   try {
  //       await Axios.get(`${axios.host}${axios.endpoints.shareVideo}?fe_album_id=${album.fe_album_id}`, {
  //           headers: {
  //             'Authorization': `Bearer ${await AsyncStorage.getItem('Token')}`
  //           }
  //       })
  //       .then((res) => {
  //           setVideosComponent(res.data)
  //           setVideoLoading(false)
  //       })
  //       .catch((err) => {
  ...
  const getSelected = contact => selectedVideos.includes(contact.fe_video_id);
  ...
  const selectItems = item => {
    setCheck(true)
    if (selectedVideos.includes(item.fe_video_id)) {
      const newListItems = selectedVideos.filter(
        listItem => listItem !== item.fe_video_id,
      );
      return setSelectedVideos([...newListItems]);
    }
    setSelectedVideos([...selectedVideos, item.fe_video_id]);
  };
        ...
        return(
            <View style={styles.container}>
                <View style={styles.firsthalf}>
                    ...
                    <Text style={styles.headingText2}>Start adding videos to your album by tapping the '+' button below.</Text>
                </View>
                <View style={styles.secondHalf}>
                    <View style={styles.hintContainer1}>
                        <View style={styles.hintContainer2}>
                            <Text style={styles.hintText}>Start adding videos to your album</Text>
                        </View>
          ...
          return(
            ...
            <Pressable onPress={deSelectItems} style={{flex: 1}}>
                <FlatList
                    ...
                    keyExtractor={item => item.fe_video_id}
                />
            </Pressable> :
            <FlatList
              numColumns={2}
              ...
              renderItem={({item})=>{
                  return(
                    <TouchableOpacity style={styles.albumContainer}
                        ...
                        onPress={()=>{
                            navigate('ShowVideos', {video: item})
                        }}>
                        {
                          Platform.OS === 'android' ?
                          (
                            <ImageBackground 
                              ...
                              source={{uri: `file://${item.cover_path}`}} 
                              resizeMode='cover'>
